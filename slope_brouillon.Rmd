...
La génération des données commence par la construction d'une matrice orthogonale X, obtenue à travers la décomposition QR : X=QR, où Q est une matrice orthogonale et R est une matrice triangulaire supérieure. Ensuite, le vecteur y est défini comme une combinaison linéaire des colonnes de X, auquel est ajouté un terme d'erreur gaussienne : y=Xβ+ϵ, où ϵ est un terme d'erreur gaussienne.

Nous avons adapté notre plan de simulation par rapport à celui présenté dans l'article de Bogdan et al. En particulier, nous utilisons λBH au lieu de λgaussien pour le contrôle du taux de faux découverte (FDR). Cette décision découle de la nécessité d'explorer l'efficacité de λBH dans un contexte de matrices orthogonales, en comparaison avec les matrices de design présentées dans l'article. Ainsi, nous visons à comprendre les performances de cette méthode dans des conditions spécifiques.
...
Pour la production de la figure 1, nous utilisons une matrice de design XX qui est orthogonale, ce qui signifie que XTX=Ip, où Ip est la matrice identité de taille p×p. Comme mentionné précédemment, nous sommes dans un cadre de régression linéaire où X est orthogonale. Ainsi, la variable réponse y_tilde peut être exprimée comme y_tilde = X^t Y= βZ ∼ N(β,0).

Dans cette perspective, le problème peut être considéré comme un problème de tests multiples, où nous utilisons la méthode de Bonferroni. Nous rejetons l'hypothèse nulle si ∣y_tilde_j2/σ > ϕ^{−}1(1−α/(2p)). En outre, comme nous utilisons la solution de Lasso avec une matrice de design orthogonale, il existe une solution explicite où βj=0 si et seulement si ∣_tilde_jj∣<λ. Ainsi, nous pouvons utiliser le seuillage Lasso avec ∣y_tilde_j∣  > λ_bonf = σ ϕ^{−}1(1−α/(2p)).
...
for (k_index in seq_along(k_values)) {
  ...
  for (sim in 1:n_sims) {
    ...
    #lasso_bonf_fit <- SLOPE(X,y, alpha = 'estimate', lambda = rep(lambda_bonf, p))
    beta_lasso_bonf <- lasso_bonf_fit$beta[,1,1]
    #beta_lasso_bonf <- lasso_bonf_fit$coefficients[2: (length(beta_true)+1)]
...
```{r}
# Graphe FWER, on veut compter, pour chaque simulation (on en fait 100) si beta compte un faux positif
...
Dans cette section, nous présentons le graphique de FDR, MSE et puissance associé à notre étude. Il est important de souligner que notre approche diffère de celle de l'article de référence, car nous utilisons lambda_BH au lieu de lambda_gaussien.
...
```{r, include = F}
# Paramètres
n <- 1500 # Nombre d'observations
p <- 1500 # Nombre de variables explicatives
num_sims <- 10
k_values <- 0:30 # Nombre de signaux véritables variant de 1 à 50
...
for (k_index in seq_along(k_values)) {
  ...
  total_fdr_adaptive_lasso <- 0  # Initialisation de la somme des FDR pour Adaptive Lasso
  ...
  for (sim in 1:num_sims) {
    ...
    # (3) Adaptive Lasso
    adaptive_lasso_fit <- glmnet(X, y, lambda = lambda_cv)
    ...
    total_fdr_adaptive_lasso <- total_fdr_adaptive_lasso + length(indices[indices > k]) / max(1,length(indices))

    # Calcul de la puissance pour l'Adaptive Lasso et ajout à la somme totale
    total_power_adaptive_lasso <- total_power_adaptive_lasso + length(indices[indices <= k]) / max(1,k)
  ...
  average_fdr_adaptive_lasso <- total_fdr_adaptive_lasso / num_sims
...
for (i in 1:3) {
  ...
  plot_data <- data.frame(k = rep(k_values[1:27], 4),
                          value = c(results[1:27, 1, i], results[1:27, 2, i], results[1:27, 3, i], results[1:27, 4, i]),
                          Method = factor(rep(c("Lasso with lambdaBF", "Lasso with lambdaCV","Adaptive Lasso", "SLOPE"), each = length(k_values[1:27]))))
...
#pdf_file <- "figures/graphique.pdf"
...
```
Observations :
En observant les résultats, nous constatons que le contrôle du FDR est effectivement maintenu, comme le prédit le théorème, avec une matrice orthogonale. De plus, nous notons que le seuil de rejet dépend de la valeur de αα, ce qui est en accord avec les observations faites dans l'article de référence.
...
calculate_mu <- function(I,J, k, sigma_matrix) {
  
  # Calcul de la norme euclidienne de chaque colonne de sigma_matrix^(-1/2)
  c <- sqrt(sum(sigma_matrix[1]^2)) # changer terme de normalisation (norme d'une ligne de SIGMA^(-1/2))
...
calculate_inverse_sqrt_matrix_diag <- function(A) {
  ...
  return(P %*% D_sqrt_inv %*% t(P))            # Retourner A^(-1/2)
}
...
step_up <- function(statistics, lambda){
  ...
  sorted = func$x
  indices = func$ix
  s = max(which(sorted >= lambda), 0) #vérifier ce que ça renvoie
  return(c(s, indices[1:s]))
}
# sum(beta_slope[(k+1):length(beta_true)] > 1e-10) / max(1,sum(beta_slope > 1e-10))*100
# Calcul du FDR
FDR_BH <- function(k,indices){
  
  ind = indices[indices > k]
  fdr <- length(ind) / max(1,length(indices))
  return(fdr)
}
# Calcul de la puissance
power_func <- function(k, indices){
  ind <- indices[indices <= k]
  power <- length(ind) / k
  return(power)
}
...
Dans cette section, nous comparons le contrôle du FDR entre les méthodes SLOPE Gaussien et StepBH dans le contexte de notre problème. Comme expliqué dans le rapport, ce problème induit une dualité entre les tests multiples et la régression linéaire.

Dans le cas des tests multiples, nous traitons un cas gaussien avec une matrice de corrélations positives. Dans cette configuration, nous pouvons appliquer la méthode de Benjamini-Hochberg (BH) car nous sommes dans un cadre où la proportion de vrais nulls est proportionnelle au rang de l'hypothèse alternative. Ainsi, BH contrôle le FDR de manière appropriée.
...
k_values <- 0:40
sigma_matrix <- calculate_sigma_matrix(sigma_tau_squared, sigma_z_squared, I, J) # -1/2
...
for (k_index in seq_along(k_values)) {
  ...
  for (i in 1:num_sims) {
    ...
    if (k==0){
      ...
      mu = calculate_mu(I, J, k, sigma_invsquared)
    }
    ...
    # Methode SLOPE
    epsilon <- mvrnorm(n = 1, mu = rep(0, I), Sigma = diag(I))
    if (k==0){
      y_tilde = epsilon
    }
    else{
      y_tilde = sigma_invsquared %*% mu + epsilon
    }
...
par(mfrow = c(1, 2))
hist(fdr_bh_values, main = "Répartition du FDP (BH)", xlab = "FDP", col = "lightblue", border = "black", xlim = c(0, 1))
hist(fdr_slope_values, main = "Répartition du FDP (SLOPE)", xlab = "FDP", col = "lightgreen", border = "black", xlim = c(0, 1))
#dev.off()
```
